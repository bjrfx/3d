...
const CENTROID_DEPTH_SENSITIVITY = 9;
const GRAB_DISTANCE_DEPTH_SENSITIVITY = 34;
const DEPTH_OFFSET_LERP = 0.28;
...
export interface DragContext {
  ...
  grabDepth: number;
  initialHandZ: number;
  initialGrabDistance: number;
  initialObjectZ: number;
  smoothedDepthOffset: number;
  cameraRight: Vector3;
  cameraUp: Vector3;
...
export const createDragContext = (): DragContext => ({
  ...
  grabDepth: 0,
  initialHandZ: 0,
  initialGrabDistance: 0,
  initialObjectZ: 0,
  smoothedDepthOffset: 0,
...
export const beginGrab = (
  ...
  drag.initialGrabDistance = hand.grabDistance;
  drag.initialObjectZ = object.position.z;
  drag.smoothedDepthOffset = 0;
...
export const computeDragTarget = (
  ...
  const centroidDepthDelta = drag.initialHandZ - hand.centroid.z;
  const grabDepthDelta = hand.grabDistance - drag.initialGrabDistance;
  const rawDepthOffset =
    centroidDepthDelta * CENTROID_DEPTH_SENSITIVITY +
    grabDepthDelta * GRAB_DISTANCE_DEPTH_SENSITIVITY;
  drag.smoothedDepthOffset = MathUtils.lerp(drag.smoothedDepthOffset, rawDepthOffset, DEPTH_OFFSET_LERP);
  ...
  outTarget.addScaledVector(drag.cameraForward, drag.smoothedDepthOffset);
